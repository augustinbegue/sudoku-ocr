    ...
    else
    {
        for (col = 0; col < sizeMatrix; col++)
        {
            ...
            sum += (double)(minorMatrix[0][col] * pow(-1, col)
                            * Determinte(
                                newMinorMatrix, (sizeMatrix - 1))); // function
        }
    }
    return sum;
} // end function
...
void Transpose(double cofactorMatrix[9][9], double sizeMatrix,
    double determinte, double coutMatrix[9][9], double transposeMatrix[9][9])
{
    int row, col;
...
void inverse_mat(
    double cinMatrix[9][9], double coutMatrix[9][9], int sizeMatrix)
{
